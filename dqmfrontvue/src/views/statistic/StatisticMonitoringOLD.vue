<template>
    <shell-with-loading :loading="loading" style="height: 100vh;">
        <filters-bar @sendData="updateData" @clear="clear">
            <div class="flex-item flex-row FIL_item">
                <div class="flex-item mr-3 flex-row FIL_item_1_per">
                    <f-chain-select
                ...
                <div class="flex-item mr-3 FIL_item_2_per">
                        <f-multi-select
                ...
                <div class="flex-item mr-3 FIL_item_3_per"> 
                    <f-multi-select
                        :field="'region__name'"
...
export default {
    ...
    data() {
        ...
        return {
            ...
            interpritateDisableRegion: {
                true: ["passport__code", "passport__name"],
            ...
            tabs: [
                {
                    text: "Динамика в разрезе регионов",
                    sort: {},
                    disableRegion: false,
                },
                {
    ...
    methods: {
        ...
        async updateData() {
            ...
            if (this.changedate) {
                ...
                const givenFilt = this.$store.getters[`filterslogic/mult/${this.includeField}`];
                this.filts =
        ...
        async clickRow(e, row) {
            ...
            this.headSubModal = this.disableRegion? `${row.item['passport__code']}. ${row.item['passport__name']}` : `${row.item['region__tax_code']}. ${row.item['region__name']}`;
            let subFilt = this.disableRegion? {'passport__code': row.item['passport__code']}: {'region__tax_code': row.item['region__tax_code']};
    ...
    computed: {
        headers() {
            ...
            return createHeader(this.disableRegion);
    ...
    mounted() {
        ...
        // this.disableRegion = this.$store.getters.region==null ? false : true;
        this.setFields({

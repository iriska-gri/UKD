...
class DescriptionData(models.Model):
    ...
    responsible = models.ForeignKey(ResponsibleMonitoring, on_delete=models.SET_NULL, verbose_name="Ответственный", blank=True, null=True)
    sub_system = models.ManyToManyField(SubSystem, verbose_name="Подсистема")
    criticality = models.ForeignKey(Criticality, on_delete=models.SET_NULL, verbose_name="Критичность", blank=True, null=True)
    priority = models.ForeignKey(Priority, on_delete=models.SET_NULL, verbose_name="Приоритет", blank=True, null=True)
...
class ListDataFBone(models.Model):
    ...
    class Meta:
        ...
        verbose_name = "Список ФБ1"
        verbose_name_plural = "Список ФБ1"
...
#     date_of_holliday = models.DateField(verbose_name="Выходные", blank=True, null=True, unique=True)


#     def __str__(self):
#         return str(self.date_of_holliday)
...
class FAQFiles(models.Model):
    name = models.CharField(verbose_name="Название", max_length=1000)
    attachmentfile = models.FileField(verbose_name="Файл",upload_to="docfiles/%Y/%m/%d")
    dock = models.ForeignKey(FAQ, on_delete=models.CASCADE, null=True, related_name='faq_files')
    class Meta:
        
        verbose_name = "Файл ЧЗВ"
        verbose_name_plural = "Файлы ЧЗВ"
        default_related_name = 'faq_files'

...
from .models import Dynamics, ListDataFBone, Passport, DescriptionData, ResponsibleMonitoring, Priority, SubSystem, Criticality, JustifyFiles
...
class JustifyFilesSerializer(serializers.ModelSerializer):
    class Meta:
            model = JustifyFiles
            fields = '__all__'

class Methodolog(serializers.ModelSerializer):
    class Meta:
            model = FNSUser
            fields = ['first_name', 'last_name']


class JustifyDataFBoneSerializer(serializers.ModelSerializer):

    passport__code = serializers.CharField()
    passport__name = serializers.CharField()
    region__tax_code = serializers.CharField()
    region__name = serializers.CharField()
    justify__date_of_justification = serializers.DateField()
    justify__description = serializers.DateField()
    justify__reason__name = serializers.CharField()
    justify__methodologist_note = serializers.CharField()
    justify__agreement_date = serializers.DateField()
    justify__fix_status = serializers.BooleanField()
    justify__pk = serializers.IntegerField()
    # justify__methodologist__last_name = serializers.CharField()
    # justify__methodologist__first_name = serializers.CharField()
    # justify__methodologist__middle_name = serializers.CharField()
    justify__methodologist=serializers.SerializerMethodField('get_meth_name')
    justify__justifiles = serializers.SerializerMethodField('get_meth_files')
    # justify__justifiles = serializers.StringRelatedField(many=True)

    def get_meth_name(self, obj):        
        if obj['justify__methodologist']:
            nob=FNSUser.objects.get(pk=obj['justify__methodologist'])
            return f"{nob.last_name} {nob.first_name[0]}. {nob.middle_name[0]}."
        return None

    def get_meth_files(self, obj):  
        #!!!! Мне кажется как-то так        
        fil=list(JustifyFiles.objects.filter(justif=obj['justify__pk']))
        li = []
        if len(fil):
            li = []
            for f in fil:
                li.append({'path':str(f.file),
                            'author':f.author
                })
            
          
        # fil='Сюда нужно впиндюрить запрос по первичному ключу'
        return li

    class Meta:
        model = ListDataFBone
    #  'justify__justifiles',
        fields = ('id', 'date_unloading', 'passport__code','passport__name', 'region__tax_code', 'region__name','inn', 'ogrn', 'fid', 'justify__reason__name',  'justify__date_of_justification', 'justify__description','justify__fix_status','justify__agreement_date','justify__methodologist','justify__methodologist_note','justify__pk','justify__justifiles',)
